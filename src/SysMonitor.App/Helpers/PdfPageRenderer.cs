...
{
    /// <summary>
    /// Renders a PDF page to PNG image bytes, one pixel per DIP times <paramref name="scale"/>. Annotations drawn
    /// on the image are stored in these pixels (see PdfPageGeometry.CanvasCoordinateScale), so the size must
    /// follow the page exactly: no minimum size, no aspect change.
    /// </summary>
    public static async Task<byte[]?> RenderPageAsync(string filePath, int pageNumber, double scale = 1.0)
    {
        try
        {
            return await PdfPageRasterizer.RenderPngAsync(filePath, pageNumber, scale);
        }
        catch (Exception ex)
        {
